...
// Global variables
// ==============================
int exitFlg;   // flag for exit, set to 1 to stop

#define MARQUEE_WIDTH 20
#define MARQUEE_MAXLEN 40 // max length for marquee
#define MAX_INSERT (MARQUEE_MAXLEN - 4) // 2 for header, 2 for footer
char insrtBuf[MAX_INSERT + 1] = ""; // buffer for inserted chars
int insrtLen = 0; // number of chars in insrtBuf
int strtIdx = 0; // current leftmost index
char marqee[MARQUEE_MAXLEN + 1] = ">><<"; // display string
int marqLen = 4; // initial length (header/footer)
int rollDirecton = 1; // 1 is left, -1 is right
...
int main(void)
{
    ...
    // Program loop
    while(exitFlg==0)  // keep going until exitFlg is set
    {
        GetInput();
        RunLogic();
        DrawScreen();
...
void GetInput(void)
{
    ...
    // check for input
    if (MacUILib_hasChar()) {
        char ch = MacUILib_getChar();
        if (ch == '#') { // '#' key for exit
            exitFlg = 1;
        } else if (ch == ' ') { // space bar changes direction
            ToggleDirection();
        } else if (((ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z') || (ch >= '0' && ch <= '9')) && insrtLen < MAX_INSERT) {
            insrtBuf[insrtLen++] = ch;
            insrtBuf[insrtLen] = '\0';
        }
    }
...
void RunLogic(void)
{
    ...
    // [TODO]: Implement the above pseudocode logic
        // Build the current marquee string: header + inserted chars + footer
    // build the marqee string
    int k = 0;
    int j;
    marqee[k++] = '>';
    marqee[k++] = '>';
    for (j = 0; j < insrtLen; ++j) {
        marqee[k++] = insrtBuf[j];
    }
    marqee[k++] = '<';
    marqee[k++] = '<';
    ...
    // update start index
    if (rollDirecton == 1) {
        strtIdx = (strtIdx + 1) % marqLen;
    } else {
        strtIdx = (strtIdx - 1 + marqLen) % marqLen;
    }
}

// Toggle marquee rolling direction
void ToggleDirection(void) {
    // toggle direction
    rollDirecton = rollDirecton * -1;
}
...
void DrawScreen(void)
{
    ...
    int i;
    int idx;
    // print 20 chars from strtIdx, wrap if needed
    for (i = 0; i < MARQUEE_WIDTH; ++i) {
        idx = (strtIdx + i) % marqLen;
        MacUILib_printf("%c", marqee[idx]);
    }
    MacUILib_printf("\n====================\n");
